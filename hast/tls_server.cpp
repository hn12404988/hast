...
bool tls_server::msg_recv(const short int thread_index){
...
inline void tls_server::echo_back_msg(const short int thread_index, std::string &msg){
	ssl_mx.lock();
	SSL_write(ssl[thread_index], msg.c_str(), msg.length());
	ssl_mx.unlock();
}

inline void tls_server::echo_back_msg(const short int thread_index, const char* msg){
	ssl_mx.lock();
	SSL_write(ssl[thread_index], msg, strlen(msg));
	ssl_mx.unlock();
}

inline void tls_server::echo_back_error(const short int thread_index, std::string msg){
	if(msg[0]=='[' || msg[0]=='{'){
		msg = "0{\"Error\":"+msg+"}";
	}
	else{
...
inline void tls_server::echo_back_sql_error(const short int thread_index){
	ssl_mx.lock();
	SSL_write(ssl[thread_index], "0{\"Error\":\"SQL\"}",16);
	ssl_mx.unlock();
}

inline void tls_server::echo_back_result(const short int thread_index, bool error){
	ssl_mx.lock();
	if(error==true){
