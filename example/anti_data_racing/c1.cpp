...
int main(){
	...
	for(;count>0;--count){
		...
		if(error_flag==0){
			std::cout << "/****** c1 fireNforget Success ********/" << std::endl;
		}
		else{
			std::cout << "/****** c1 fireNforget Fail ********/" << std::endl;
		}
	}
	std::cout << "c1 waiting for all fireNforget are processed by server"<< std::endl;
	if(client.join(to_server)==true){
		std::cout << "c1 join fireNforget successfully" << std::endl;
	}
	else{
		std::cout << "c1 fail on join fireNforget" << std::endl;
	}
	return 0;
}
